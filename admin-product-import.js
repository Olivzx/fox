(()=>{
  ...
  async function importProduct(form){
    ...
    try{
      const client=await getClient();const {data:{session}}=await client.auth.getSession();if(!session?.access_token)throw new Error('Sua sessão administrativa expirou. Entre novamente.');
      ...
      const d=json.data||{};const set=(sel,val)=>{const el=form.querySelector(sel);if(el&&val!==null&&val!==undefined&&String(val)!=='')el.value=val};
      ...
      let note=form.querySelector('.affiliate-import-note');if(!note){note=document.createElement('div');note.className='affiliate-import-note';form.querySelector('#purl')?.closest('.field')?.appendChild(note)}note.innerHTML=`<span>✓ Dados encontrados automaticamente</span><small>${esc(d.marketplace||'Marketplace detectado')} · confira os campos antes de salvar.</small>`;
  ...
  function enhance(form){if(form.dataset.importEnhanced)return;form.dataset.importEnhanced='1';const field=form.querySelector('#purl')?.closest('.field');if(!field)return;const btn=document.createElement('button');btn.type='button';btn.id='fetchProductData';btn.className='table-action primary affiliate-import-btn';btn.textContent='↙ Preencher automaticamente';btn.addEventListener('click',()=>importProduct(form));const help=document.createElement('small');help.className='affiliate-import-help';help.textContent='Cole o link da Shopee, Mercado Livre, Amazon, AliExpress ou outra loja. A imagem e os dados disponíveis serão preenchidos para você revisar.';field.appendChild(btn);field.appendChild(help)}
  const scan=()=>document.querySelectorAll('#productForm').forEach(enhance);new MutationObserver(scan).observe(document.body,{childList:true,subtree:true});scan();
})();